...
Build a self-hosted inbound AI voice agent using the LiveKit Agents framework. The agent answers customer calls, converses naturally, and logs call details. This guide is instructions to get Claude Code to behave the way I want.
...
- **Database/Logs:** Airtable (Personal Access Token & Base ID)
- **Hosting/Deployment:** Hostinger VPS (Ubuntu)

---

# Running the Project

1. Copy `.env.example` to `.env` and fill in your API keys (LiveKit, Deepgram, OpenAI, ElevenLabs, Twilio, Airtable).
2. Install dependencies: `pip install -r requirements.txt`
3. Run the agent: `python agent.py` (or start the LiveKit worker/dev server).

---

# File Structure

- `agent.py` → Main application code and voice pipeline logic
...
When calling external APIs or services (LiveKit, OpenAI, Airtable, etc.):
